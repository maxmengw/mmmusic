...
import { AddMusicToExampleDto, DeleteMusicFromExampleDto } from "@shared/types/MusicDto";
...
export async function validateAddMusic(
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> {
    const { error, value } = addToExampleSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    ...
    req.body = value;
    const { name, example } = req.body as AddMusicToExampleDto;
    ...
    try {
        ...
        if (currentExamples.includes(example.trim())) {
            res.status(400).json(errorResponse("A music example with the same name already exists.", "VALIDATION_ERROR"));
            return;
        }
...
export async function validateDeleteMusic(
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> {
    const { error, value } = deleteFromExampleSchema.validate(req.body, { abortEarly: false, stripUnknown: true });
    ...
    req.body = value;
    const { name, example } = req.body as DeleteMusicFromExampleDto;
    ...
    try {
        ...
        if (!currentExamples.includes(example.trim())) {
            res.status(404).json(errorResponse("Music not found in this singer.", "NOT_FOUND"));
            return;
        }
